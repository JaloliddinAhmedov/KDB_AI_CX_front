...
export const TransferWidget: React.FC<TransferWidgetProps> = ({
  ...
  const [purpose, setPurpose] = useState<string>(initialPayload.purpose || "Direct transfer");
  ...
  const handleSelectSenderAccount = (accId: string) => {
    setSelectedAccountId(accId);
    const target = accounts.find(a => a.id === accId);
    if (target) {
      setCurrency(target.currency as 'UZS' | 'USD');
    }
  };
  ...
  const steps = [
    { num: 1, text: "1/4 Verifying recipient banking credentials & card status..." },
    { num: 2, text: `2/4 KDB Compliance, AML & Sanctions screening: Approved (Risk: ${currentCompliance.riskScore}/100)...` },
    { num: 3, text: "3/4 Routing transaction via Central Bank & KDB Clearing Gateway..." },
    { num: 4, text: "4/4 Transfer successfully executed! Funds credited to recipient." }
  ];

  const handleStartTransfer = () => {
    if (amount <= 0 || !beneficiaryName.trim() || !cardOrAccount.trim() || hasInsufficientFunds) return;
    ...
    setTimeout(() => {
      ...
      const donePayload: TransferPayload = {
        ...
        amount,
        currency,
        purpose: purpose.trim() || `Transfer to ${beneficiaryName}`,
        commission: 0,
        status: 'Completed',
  ...
  const handleCopyReceipt = () => {
    ...
    const text = `KDB Bank Uzbekistan Official Transaction Receipt\nReceipt ID: #${completedPayload.receiptId}\nReceiver: ${completedPayload.toBeneficiary}\nCard/Account: ${completedPayload.toCardOrAccount}\nDebit Account: ${selectedAccount.name}\nAmount: ${formatCurrency(completedPayload.amount, completedPayload.currency)}\nBank Fee: 0 UZS (0% KDB Free)\nStatus: Completed (Success)\nDate: ${completedPayload.timestamp}\nAML & Compliance: Approved (Risk Score: ${completedPayload.compliance?.riskScore || 2}/100)`;
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h4 className="text-xs sm:text-sm font-bold text-slate-900">KDB Instant Money Transfer</h4>
                <p className="text-[10px] text-slate-400">Select sender card/account, receiver, and amount</p>
              </div>
            </div>
          ...
          {/* SENDER ACCOUNT SELECTION (DEBIT) */}
          <div className="space-y-1.5">
            <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500">
              1. Choose Sender Account / Card (Debit)
            </label>
              ...
              {accounts.map(acc => {
                ...
                return (
                  <button
                    key={acc.id}
                    type="button"
                    onClick={() => handleSelectSenderAccount(acc.id)}
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${acc.color || 'from-indigo-600 to-indigo-800'} text-white flex items-center justify-center shrink-0 shadow-2xs`}>
                        <CreditCard className="w-3.5 h-3.5" />
                      </div>
                      <div className="min-w-0">
                        <p className="text-[11px] font-bold text-slate-900 truncate">{acc.name}</p>
                        <p className="text-[10px] font-mono text-slate-500">{acc.cardMask || acc.accountNumber.slice(-8)}</p>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      ...
                      <span className="text-[9px] font-bold uppercase px-1 rounded bg-slate-200 text-slate-700">
                        {acc.currency}
                      </span>
                    </div>
                  </button>
          ...
          {/* RECEIVER SELECTION (SAVED BENEFICIARIES) */}
          <div className="space-y-1.5 pt-1">
            <div className="flex items-center justify-between text-[10px] font-bold uppercase text-slate-500">
              <span className="flex items-center gap-1">
                <Users className="w-3 h-3 text-indigo-600" />
                2. Choose Receiver / Beneficiary
              </span>
              <button
                type="button"
                onClick={handleSwitchToManual}
                className="text-indigo-600 hover:text-indigo-800 font-bold flex items-center gap-0.5 cursor-pointer normal-case text-[11px]"
              >
                <PlusCircle className="w-3 h-3" />
                <span>+ New Card or Account</span>
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs">
            ...
            <div className="p-2.5 rounded-xl border border-slate-200/80 bg-slate-50/50 space-y-1">
              <div className="flex items-center justify-between text-[10px] font-bold uppercase text-slate-400">
                <span>Card or Account Number</span>
          ...
          <div className="flex items-center gap-2 px-1">
            ...
            <label htmlFor="saveBeneficiaryCheck" className="text-[11px] text-slate-600 cursor-pointer flex items-center gap-1 select-none">
              ...
              <span>Save this receiver to contacts for future transfers</span>
            </label>
          </div>
          ...
          <div className="bg-gradient-to-br from-indigo-50/70 to-slate-50 border border-indigo-100 rounded-xl p-3.5 space-y-2.5">
            ...
            {hasInsufficientFunds && (
              <div className="flex items-center gap-1.5 text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-2 text-[11px] font-medium">
                ...
                <span>Insufficient funds on selected sender account ({formatCurrency(selectedAccount.balance, selectedAccount.currency)})</span>
              </div>
            )}

            <div className="flex justify-between items-center text-[10px] text-slate-500 pt-1 border-t border-indigo-100/60">
              <span>Available balance: <strong>{formatCurrency(selectedAccount.balance, selectedAccount.currency)}</strong></span>
              <span>Bank Fee: <strong className="text-emerald-600 font-bold">0 UZS (0% KDB Free)</strong></span>
            </div>
          </div>
          ...
          <div className="p-2.5 rounded-xl border border-slate-200/80 bg-slate-50/50 space-y-1 text-xs">
            ...
            <input
              ...
              placeholder="e.g., Invoice settlement, consulting service..."
          ...
          <div className="bg-emerald-50/60 border border-emerald-200 rounded-xl p-3 flex items-start gap-2.5 text-[11px] text-emerald-900">
            ...
            <div className="space-y-0.5">
              <p className="font-bold text-emerald-950">Automated Security & AML Screening: Passed</p>
              <p className="text-emerald-800 leading-relaxed text-[10px]">
                OFAC, UN and Central Bank of Uzbekistan sanction databases scanned for {beneficiaryName}. Transaction approved.
              </p>
            </div>
          </div>
        ...
        <div className="space-y-4 animate-in zoom-in-95 duration-200">
          ...
          <div className="bg-emerald-600 rounded-xl p-4 text-white text-center shadow-xs space-y-1">
            ...
            <p className="text-[11px] text-emerald-100">
              Transaction verified and cleared by KDB Bank Core System
            </p>
          </div>
          ...
          <div className="border border-dashed border-slate-300 rounded-xl p-4 bg-slate-50/60 space-y-3">
            <div className="flex items-center justify-between border-b border-slate-200/80 pb-2.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-800">Official Bank Transaction Receipt</span>
              </div>
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between items-center text-slate-600">
                <span>Receiver / Beneficiary:</span>
              ...
              <div className="flex justify-between items-center text-slate-600">
                <span>Debit Account (Sender):</span>
                <span className="font-medium text-slate-700">{selectedAccount.name}</span>
              </div>

              <div className="flex justify-between items-center text-slate-600">
                <span>Bank Fee:</span>
                <span className="font-bold text-emerald-600">0 UZS (0% KDB Free)</span>
              </div>
              ...
              <div className="flex justify-between items-center text-slate-600">
                ...
                <span className="text-emerald-700 font-bold flex items-center gap-1">
                  ...
                  Approved (Risk Score: {completedPayload.compliance?.riskScore || 2}/100)
                </span>
              </div>
            </div>
          </div>
